...
#| fig-width: 10
#| fig-height: 11
#| fig-cap: "Visualisation of the static events (with a time window of 10 seconds) inside the buffer for different cluster distances and buffer sizes for strava 1"
#| label: fig-10s-strava1
...
for (md in max_dist_liste) {
  ...
  filtered <- Strava_1_10_stops_list |> 
...
wrap_plots(plot_liste, ncol = 3) + 
  ...
  plot_annotation(
    title = "Strava 1 - 9 combinations of clustering and buffering around waypoints",
...
#| fig-width: 10
#| fig-height: 11
#| fig-cap: "Visualisation of the static events (with a time window of 20 seconds) inside the buffer for different cluster distances and buffer sizes for swisstopo 2"
#| label: fig-20s-swisstopo2
...
for (md in max_dist_liste) {
  ...
  filtered <- Swisstopo_2_20_stops_list |> 
    filter(max_dist_setting == md)
...
wrap_plots(plot_liste, ncol = 3) + 
  plot_layout(guides = "collect") + 
  plot_annotation(
    title = "Swisstopo 2 - 9 combinations of clustering and buffering around waypoints",
    subtitle = "Temporal window for static events = 20 seconds | Cluster-Distanz (MD) | Waypoint-Buffer (Buffer)"
  )
```
...
wrap_plots(plot_liste, ncol = 3) + 
  plot_layout(guides = "collect") + 
  plot_annotation(
    title = "Posmo 2 - 9 combinations of clustering and buffering around waypoints",
    subtitle = "Temporal window for static events = 40 seconds | Cluster-Distanz (MD) | Waypoint-Buffer (Buffer)"
  )
```