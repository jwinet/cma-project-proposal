...
subtitle: Using segmentation algorithms, clustering algorithms and buffering around the ground truth points to classify stops
...
Furthermore, a distance analysis evaluates these centroids against the ground truth waypoint locations. Multiple distance buffers (5, 10 and 15 metres) are generated around waypoints to identify spatial intersections. To avoid multiple true positives per buffer, classified stops within a buffer are filtered and combined into a single stop using their centroids. This ensures data consistency and simplifies data analysis and interpretation.
...
Precision ($\frac{TP}{TP+FP}$) measures the proportion of correct predicted stops, while recall ($\frac{TP}{TP+FN}$) indicates if all ground truth stops were identified [@powers2011evaluation]. The F1-score ($2 \times \frac{\text{Precision} \times \text{Recall}}{\text{Precision} + \text{Recall}}$) combines both parameters into a single value between 0 and 1 [@powers2011evaluation]. For all three metrics, values closer to 1 indicate higher accuracy.
...
@fig-10-window, [-@fig-20-window], [-@fig-30-window] and [-@fig-40-window] show the results of the static event identification for the four temporal windows of 10, 20, 30 and 40 seconds. Because the PosmoProject data were not captured at continuous intervals, no neighbouring fixes were detected for some fixes in the 10- and 20-second temporal windows. These points are plotted as "Unknown" and cannot be used for further analysis. 

```{r 10s Window}
#| fig-width: 10
#| fig-height: 11
#| fig-cap: "Visualisation of static events on all tracks within a 10-second time window."
#| label: fig-10-window
...
#| fig-width: 10
#| fig-height: 11
#| fig-cap: "Visualisation of static events on all tracks within a 20-second time window."
#| label: fig-20-window
...
#| fig-width: 10
#| fig-height: 11
#| fig-cap: "Visualisation of static events on all tracks within a 30-second time window."
#| label: fig-30-window
...
#| fig-width: 10
#| fig-height: 11
#| fig-cap: "Visualisation of static events on all tracks within a 40-second time window."
#| label: fig-40-window
...
#| fig-width: 8
#| fig-height: 10
#| fig-cap: "Visualisation of the static events (time window of 10 seconds) inside the buffer for different cluster distances and buffer sizes for Strava 2."
...
for (md in max_dist_liste) {
  ...
  for (buf in wp_buffer_liste) {
    ...
    show_leg <- (md == max_dist_liste[1] & buf == wp_buffer_liste[1]) # Only show the legend on the very first plot, hide it for the other 8
    
    p <- ggplot() +
      geom_sf(data = st_buffer(wp_olten_clean, buf), 
              aes(fill = "Waypoint Buffer Area"), 
              alpha = 0.50, 
              color = NA,
              key_glyph = "point",
              show.legend = show_leg) +
      geom_sf(data = filtered, 
              aes(color = factor(.data[[current_col]], levels = c("0", "1"))), 
              size = 1,
              show.legend = show_leg) +
      scale_fill_manual(
        values = c("Waypoint Buffer Area" = "gray70"),
        name = NULL,
        guide = guide_legend(
          override.aes = list(
            shape = 21,       # Filled circle shape
            size = 4,         # Makes the circle larger and clearer in the legend
            alpha = 0.50,      # Matches the map transparency
            color = NA
          ),
          order = 1
        )
...
#| fig-width: 8
#| fig-height: 8
#| fig-cap: "Visualisation of the static events (time window of 30 seconds) inside the buffer for different cluster distances and buffer sizes for PosmoProject 1."
...
Furthermore, the table shows the performance metrics precision and recall.
...
#| tbl-cap: "Parameter table evaluating out of a maximum of 88 possible ground truth waypoints across all combined tracking apps. The top 5 performing rows for recall values are highlighted in green and the bottom 5 in red."
...
From the precision and recall values, we can calculate the F1-score, which provides a further indication of performance. @fig-F1-together shows the F1-score for all the apps combined, while @fig-F1-by-App shows the F1 scores more precisely for each app individually. @fig-F1-together and [-@fig-F1-by-App] underline the results of @tbl-parameters-all-prov, that the 5-metre waypoint buffer performs worst, especially with a 10-second time window. In contrast, the best performance is achieved by combining a 15-metre buffer with a large temporal window and higher maximal clustering distances.
...
@fig-F1-by-App shows that the Strava app performs the best overall, whereas the PosmoProject app performs poorly, especially within a 10-second time window. This is also further highlighted in @tbl-app-performance. 

```{r F1-score by App}
#| fig-width: 8
#| fig-cap: "Visualisation of F1-score for the different parameters for all apps separately."
#| label: fig-F1-by-App
...
However, some of the best F1-scores for the PosmoProject and Swisstopo apps were achieved with the 40-second and the 30-second time windows, with a 10-metre clustering distance (@tbl-app-best-performance). 

```{r Top performance for each app}
#| tbl-cap: "Summary of F1-Scores per tracking app, showing the best performance alongside its optimal parameters, dataset averages and the number of perfect performances per app. The best F1-score only shows one example of a possible parameter combination for a perfect F1-score for each app, but there may also be different parameter combinations."
...
Grouping the results for the different parameters separately, it is further highlighted that the 30- and 40-seconds windows create the best F1 scores (@tbl-best-perf-tw). The different clustering distances, on the other hand, seem to not have a big influence on the F1 score, whereas the buffer size of 5 metres results in significantly lower overall F1 scores, as already mentioned above (@tbl-best-perf-CD and [-@tbl-best-perf-buff]). 

::: {#tbl-panel layout-ncol=3}

```{r time window}
#| tbl-cap: "Average performance  for each time window for segmentation."
...
```{r clustering}
#| tbl-cap: "Average performance  for each clustering distance."
...
```{r buffer}
#| tbl-cap: "Average performance  for each waypoint buffer size."
...
The following analysis relies on F1-scores, as this metric incorporates TP, FP, and FN by combining precision and recall. In @fig-F1-together, the low F1-score indicates that the 5-metre buffer is not an ideal choice for detecting stops within a certain area. This occurs because stops usually cover a wide area and exercises require a larger radius, which may not be captured by such a small buffer. Similarly, GPS inaccuracies for stop detection may be more sensitive for smaller buffers. This negative behaviour of the 5-metre buffer is also evident in @tbl-best-perf-buff. 
The results further show that the performance (F1-score) is significantly lower for 10-second intervals. However, this is most likely due to the poor performance of the PosmoProject app at this interval (@fig-F1-by-App). The PosmoProject app produces very low performance metric values at short time intervals. This is a weakness of the chosen methodology, since on average, PosmoProject provides a GPS fix every 15 seconds and there are not enough neighbours within the 10-second interval to identify static points. This results in a large number of NA mean spatial distance values. Even at 20-second intervals, some NA mean spatial distance values remain, since on average only 1 to 2 neighbours are detected within 20 seconds. Even with 40-second intervals, there is an average of only 3 neighbours. Strava and Swisstopo, on the other hand, send a signal every 1 to 2 seconds, which means that in a 10-second interval, the mean spatial distance is already calculated from ten neighbours.

Instead of calculating the mean spatial distances, we initially tried using linear interpolation to standardise the time intervals across apps. However, with long signal intervals, such as in the PosmoProject, interpolation can lead to inaccurate coordinate shifts. Thus, the PosmoProject results, which record fixes at roughly 15-second intervals, must be interpreted with caution, as there are not always enough data points for an accurate determination. They may even be meaningless in some cases, especially when a time interval contains 0 to 1 data points. Therefore, this method proved to be a significant limitation and mean spatial distances were used instead.

@tbl-parameters-all-prov shows that both precision and recall improve with a longer time interval. @tbl-app-performance also reveals the impact of the app, showing that PosmoProject has a significantly negative effect on recall values. This means that at 10-second intervals, the identification of all 15 existing waypoint buffers in Wollerau is very inaccurate due to the NA values. In this study, not only do bad recall values have a negative impact on the F1 score, but also low precision values. In @tbl-app-performance, Swisstopo return lower F1-scores at shorter time intervals. Recall remains equally good across time intervals, but precision varies. The smaller the time interval, the lower the F1-score, as fewer neighbours are available to determine correct stops and the risk of identifying incorrect stops increases. This is not only the case for Swisstopo but is rather consistent regardless of the app (@tbl-best-perf-tw).

Furthermore, it is interesting that the waypoints were created using Swisstopo, yet Strava delivers better F1-scores than Swisstopo (@tbl-app-best-performance). Strava even manages to achieve a perfect F1-score of 1 in 20 different combinations. In contrast, neither PosmoProject nor Swisstopo ever reach 1. However, the best value for Swisstopo is 0.966, which is very close. Since Strava was the only app used in Olten, it is possible that the running behaviour in Olten was more suitable for the segmentation method. For example, longer breaks were taken during stops, no interruptions occurred during the run or perhaps the app just performs better. Since both Vitaparcours are mostly in forested areas, this should not have a significant influence on comparisons between the two tracks.
The only parameter that makes hardly any difference to the results in @tbl-best-perf-CD is the cluster selection. This is because static data points are either very close to each other or far apart so that only extreme cluster sizes can make a difference.

## Conclusion

The analysis of the results was generated using precision, recall and F1-score. Several distinct patterns are evident. The buffer radius around the waypoints should not be too small, as GPS inaccuracies will have a significant impact. Therefore, a buffer radius of 5 metres is not suitable to compare stops generated from static events with ground truth points. Clusters, on the other hand, are resilient to changes in size, as long as rather small values below 15 metres are chosen, since error points are either very close together or very far apart. The method of calculating mean spatial distance values from neighbours at different time intervals ensures that longer time intervals, such as 30 or 40 seconds, provide more accurate results, since more neighbours are included in the calculation and a comparison between different sampling intervals can be guaranteed. However, this has negative consequences for the PosmoProject data, since at short intervals, such as 10 seconds, there are not enough neighbours to perform a calculation within that interval. 
