...
However, the best F1 scores for the PosmoProject and Swisstopo apps were achieved with the 40 seconds and the 30 seconds time window with a 10 metre clustering distance (@tbl-app-best-performance). 
...
In @fig-F1-together, the low F1 score indicates that the 5 meter buffer is not an ideal choice for detecting stops within a certain area. This is because the stops usually cover a "large" area and the exercises require a larger radius, which may not be captured by such a small buffer. Similarly, GPS inaccuracies for stop detection may be more sensitive for smaller buffers. This negative behaviour of the 5 metre buffer is also evident in @tbl-best-perf-buff. 
