...
ggplot(global_parameter_table, aes(x = Max_Dist, y = WP_Buffer, fill = F1)) +
  ...
  labs(title = "F1 Score across Parameters", fill = "F1")

add_metrics <- function(df) {
  df |>
    mutate(
      Precision = True_Pos / (True_Pos + False_Pos),
      Recall    = Captured / Total_GT,
...
app_summary <- master_apps %>%
  group_by(App) %>%
  summarise(
    best_F1 = max(F1, na.rm = TRUE),
    mean_F1 = mean(F1, na.rm = TRUE),
    count_F1_equal_1 = sum(F1 == 1, na.rm = TRUE)
  ) %>%
  ungroup()


Window_summary <- master_apps %>%
  group_by(Window) %>%
  summarise(
    mean_F1 = mean(F1, na.rm = TRUE),
    .groups = "drop"
