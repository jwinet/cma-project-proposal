...
The results of this analysis were generated using F1 scores. Several distinct patterns are evident. The buffer radius around the waypoints should not be too small, as otherwise GPS inaccuracies will have a significant impact or the exercise will require more space. Therefore, 5 meters is not suitable as a buffer. Clusters, on the other hand, are resilient to changes in size as long as they are not taken to extremes, since error points are either very close together or very far apart. The chosen method of calculating stepMean values from neighbors at different time intervals ensures that longer time intervals, such as 30 or 40 seconds, yield more accurate results, since more neighbors are included in the calculation. Furthermore, this has negative consequences for Posmo, since a signal is sent only every 15 seconds, and in short intervals such as 10 seconds, there are not enough neighbors to perform a calculation within that interval. Interpolation could solve this methodological problem. In contrast, Strava shows the best F1-scores, followed by Swisstopo, as both apps send signals every 1 to 2 seconds.
